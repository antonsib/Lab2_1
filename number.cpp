...
Number::operator unsigned long() {//преобразование к типу unsigned long
	try { 
		...
		int exit = 0;
         if (this->n > 10) throw 219;//219-номер ошибки,предел unsigned long-10 знаков
		 ...
		 if (exit == 1) throw 219;//если число больше предела,то выбрасываем ошибку
		...
		cout << "Error!Overflow type unsigned long!" << endl;
	}
}
...
istream& operator>> (istream &in, Number &numOne) {//ввод с консоли
	in >> numOne.n;
	numOne.setArray();
	for (int i = 0; i < numOne.n; i++) {
		in >> numOne.a[i];
...
ofstream& operator<< (ofstream& os, Number &numOne) {//вывод в файл
	/*os << numOne.n;*/
	for (int i = 0; i < numOne.n; i++)
		os << numOne.a[i];
	os << endl;
...
ifstream& operator>> (ifstream& is, Number &numOne) {//ввод из файла
